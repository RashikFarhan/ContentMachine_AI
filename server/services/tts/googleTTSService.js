...
class GoogleTTSService {
    ...
    async generateSpeech(text, voiceId, options = {}) {
        try {
            ...
            const audioDir = path.join(paths.publicDir, 'audio');
            ...
            if (apiError) {
                console.error("GoogleTTSService API Error:", apiError);
                throw new Error(`Gemini API Error: ${apiError}`);
            }
            console.error("GoogleTTSService generateSpeech error:", error.message);
            throw error;
        }
    }
