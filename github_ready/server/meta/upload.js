...
class MetaUploader {
    ...
    async uploadLatestVideo(workspaceId) {
        ...
        const filename = path.basename(videoData.video_path);
        const videoUrl = `${getPublicUrl()}/videos/${filename}`;

        const caption = `${videoData.title || 'New Reel'}\n\n${videoData.description || '#shorts #reels'}`;
        ...
        if (targetAccount.page_id) {
            try {
                ...
                form.append('source', fs.createReadStream(videoData.video_path));
