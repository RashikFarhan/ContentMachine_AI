...
class TTSManager {
    ...
    async applySpeed(relativePath, speedFactor) {
        return new Promise((resolve, reject) => {
            ...
            const scriptPath = path.join(__dirname, '../../tools/change_speed.py');

            console.log(`TTS Manager: Applying speed ${speedFactor}x on ${relativePath}`);

            const pythonProcess = spawn('python', [scriptPath, absolutePath, speedFactor.toString()]);
    ...
    async enforceDuration(relativePath, maxDuration) {
        return new Promise((resolve, reject) => {
            ...
            // public is at ../../public
            const absolutePath = path.join(__dirname, '../../public', relativePath);
            const scriptPath = path.join(__dirname, '../../tools/enforce_duration.py');

            console.log(`TTS Manager: Enforcing max duration of ${maxDuration}s on ${relativePath}`);

            const pythonProcess = spawn('python', [scriptPath, absolutePath, maxDuration.toString()]);
